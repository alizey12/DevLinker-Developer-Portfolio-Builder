...
const DashboardLayout = () => {
  ...
  return (
    <div className={`flex min-h-screen ${theme === 'dark' ? 'bg-emerald-900 text-emerald-100' : 'bg-gray-50 text-emerald-800'}`}>
      {/* Sidebar */}
      <div className={`${isCollapsed ? 'w-20' : 'w-64'} transition-all duration-300 ${
        theme === 'dark' ? 'bg-emerald-700/50 border-emerald-700/50' : 'bg-white border-gray-200'
      } border-r`}>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <header className={`flex items-center justify-between p-4 border-b ${
          theme === 'dark' ? 'bg-emerald-800 border-emerald-700' : 'bg-white border-emerald-200'
        }`}>
