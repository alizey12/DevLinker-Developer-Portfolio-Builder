...
import { FiCode, FiLayout, FiSmartphone, FiZap, FiMessageCircle, FiMail, FiDownload } from 'react-icons/fi';
...
const Home = () => {
  ...
  const handleAISubmit = (e) => {
    ...
    setIsAiTyping(true);
    setAiResponse(prev => [...prev, { text: aiMessage, fromUser: true }]);
    
    // Simulate AI response
    setTimeout(() => {
      const responses = [
        "I'd be happy to help with that!",
        "Kinza has extensive experience with that technology.",
        "That project was built using React and Node.js.",
        "She's currently available for freelance work.",
        "You can expect a response within 24 hours."
      ];
      const randomResponse = responses[Math.floor(Math.random() * responses.length)];
      setAiResponse(prev => [...prev, { text: randomResponse, fromUser: false }]);
      setIsAiTyping(false);
    }, 1500);
    ...
    <div className='w-full min-h-screen bg-emerald-950 text-white'>
      ...
      <div className='container mx-auto px-6 py-16 flex flex-col md:flex-row items-center justify-between'>
        <div className='md:w-1/2 mb-10 md:mb-0'>
          ...
          <div className='flex flex-col sm:flex-row gap-4'>
            ...
            <a 
              href='https://drive.google.com/file/d/1grc_6AjqgpVXwyG1QztYtHoUtznxpdHr/view'
              className='px-8 py-3 border border-emerald-400 hover:bg-emerald-900 rounded-full font-medium transition-colors duration-300 text-center flex items-center justify-center'
            >
      ...
      <div className='container mx-auto px-6 py-20 text-center'>
        ...
        <div className='flex flex-col sm:flex-row gap-4 justify-center'>
          <NavLink 
            to="/contact" 
            className='px-8 py-3 bg-emerald-600 hover:bg-emerald-700 rounded-full font-medium transition-colors duration-300 flex items-center justify-center'
          >
          ...
          <button 
            ...
            className="px-8 py-3 border border-emerald-400 hover:bg-emerald-900 rounded-full font-medium transition-all duration-300 flex items-center justify-center group"
          >
            <FiMessageCircle className="mr-2 group-hover:animate-bounce" /> 
      ...
      {showAI && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-gray-800 rounded-xl p-6 w-full max-w-md border border-emerald-400/30">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-xl font-bold text-emerald-100 flex items-center">
                <FiMessageCircle className="mr-2 text-emerald-400" />
                Portfolio Assistant
              </h3>
              <button 
            ...
            <div className="bg-gray-900 rounded-lg p-4 mb-4 h-64 overflow-y-auto">
              {aiResponse.length === 0 ? (
                <div className="text-center h-full flex flex-col justify-center">
                  <p className="text-emerald-200 mb-2">Hello! I'm your AI assistant.</p>
                  <p className="text-gray-400 text-sm">Ask me about Kinza's skills, projects, or availability.</p>
                </div>
              ) : (
                aiResponse.map((msg, i) => (
                  <div 
                    key={i} 
                    className={`mb-3 ${msg.fromUser ? 'text-right' : 'text-left'}`}
                  >
                    <div className={`inline-block px-4 py-2 rounded-lg ${msg.fromUser ? 'bg-emerald-600 text-white' : 'bg-gray-700 text-emerald-100'}`}>
                      {msg.text}
                    </div>
                  </div>
                ))
            ...
            <form onSubmit={handleAISubmit} className="flex gap-2">
              <input 
                ...
                className="flex-1 bg-gray-700 rounded-lg px-4 py-2 text-white focus:outline-none focus:ring-2 focus:ring-emerald-500"
                autoFocus
              />
              <button 
                type="submit"
                className="bg-emerald-600 hover:bg-emerald-700 px-4 py-2 rounded-lg flex items-center justify-center"
              >
                Send
              </button>
            </form>
