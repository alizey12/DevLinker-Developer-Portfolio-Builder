...
import { 
  FiUser, FiSun, FiMoon, FiMail, FiLock, FiGlobe, 
  FiBell, FiCreditCard, FiEdit2, FiCheck, FiX 
} from 'react-icons/fi';
...
const Settings = () => {
  ...
  const startEditingAccount = () => {
    setAccountInfo({
      ...accountInfo,
      editing: true,
      tempUsername: accountInfo.username,
      tempEmail: accountInfo.email
    });
  };
  ...
  const saveAccountInfo = () => {
    setAccountInfo({
      ...accountInfo,
      username: accountInfo.tempUsername,
      email: accountInfo.tempEmail,
      editing: false
    });
  ...
  const primaryButtonClasses = `
    w-full mt-4 py-2 px-4 rounded-lg transition-all border flex items-center justify-center
    ${theme === 'dark' 
      ? 'bg-emerald-400/70 hover:bg-primary/20 text-emerald-400 border-primary/20' 
      : 'bg-emerald-100 hover:bg-emerald-200 text-emerald-800 border-emerald-200'}
  `;

  const secondaryButtonClasses = `
    w-full py-2 px-4 rounded-lg transition-all border flex items-center justify-center
    ${theme === 'dark' 
  ...
  const themeButtonClasses = (selectedTheme) => `
    ...
    ${theme === selectedTheme || 
      ...
      ? 'bg-emerald-400/70 border border-primary/50' 
      : theme === 'dark' 
  ...
  const inputClasses = `
    w-full rounded-lg p-2 focus:ring-2 focus:ring-primary focus:border-transparent
    ${theme === 'dark' 
              ...
              {accountInfo.editing ? (
                <>
                  <div className="space-y-2">
                    <label className={`block text-sm ${textSecondary}`}>Username</label>
                    <input
                      type="text"
                      value={accountInfo.tempUsername}
                      onChange={(e) => setAccountInfo({...accountInfo, tempUsername: e.target.value})}
                      className={inputClasses}
                    />
                  </div>
                  <div className="space-y-2">
                    <label className={`block text-sm ${textSecondary}`}>Email</label>
                    <input
                      type="email"
                      value={accountInfo.tempEmail}
                      onChange={(e) => setAccountInfo({...accountInfo, tempEmail: e.target.value})}
                      className={inputClasses}
                    />
                  </div>
                ...
                <>
                  <div className="flex justify-between items-center py-2 border-b border-emerald-800/30">
                    ...
                    <span className={`font-medium ${textPrimary}`}>{accountInfo.username}</span>
                  </div>
                  <div className="flex justify-between items-center py-2 border-b border-emerald-800/30">
                    <span className={`text-sm ${textSecondary}`}>Email</span>
                    <span className={`font-medium ${textPrimary}`}>{accountInfo.email}</span>
                  </div>
                  <div className="flex justify-between items-center py-2">
                    <span className={`text-sm ${textSecondary}`}>Member Since</span>
                    <span className={`font-medium ${textPrimary}`}>{accountInfo.memberSince}</span>
                  </div>
                  <button 
          ...
          <div className={cardClasses}>
            ...
            <div className="space-y-4">
              {security.changingPassword ? (
                <>
                  <div className="space-y-2">
                    <label className={`block text-sm ${textSecondary}`}>Current Password</label>
                    <input
                      type="password"
                  ...
                  <div className="space-y-2">
                    <label className={`block text-sm ${textSecondary}`}>New Password</label>
                    <input
                      type="password"
                  ...
                  <div className="space-y-2">
                    <label className={`block text-sm ${textSecondary}`}>Confirm New Password</label>
                    <input
                      type="password"
                  ...
                  <div className="flex space-x-2">
                    <button 
                      ...
                      className={`${primaryButtonClasses} flex-1`}
                    >
                      <FiCheck className="mr-2" /> Save Password
                    </button>
                    <button 
          ...
          <div className={cardClasses}>
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-center py-2 border-b border-emerald-800/30">
                ...
                <span className={`font-medium ${textPrimary}`}>{billing.plan}</span>
              </div>
              <div className="flex justify-between items-center py-2 border-b border-emerald-800/30">
                <span className={`text-sm ${textSecondary}`}>Payment Method</span>
                <span className={`font-medium ${textPrimary}`}>{billing.paymentMethod}</span>
              </div>
              <div className="flex justify-between items-center py-2">
                <span className={`text-sm ${textSecondary}`}>Next Billing Date</span>
                <span className={`font-medium ${textPrimary}`}>{billing.nextBillingDate}</span>
              </div>
              <button 
