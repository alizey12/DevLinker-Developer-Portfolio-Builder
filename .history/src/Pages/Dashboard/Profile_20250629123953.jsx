...
const Profile = () => {
  ...
  return (
    ...
    <div className="flex flex-col md:flex-row gap-6">
      ...
      <div className='w-full slashed-zero bg-gray-900 text-white p-6'>
      <h2 className="text-xl font-semibold mb-4">Edit Profile</h2>
        <form className="space-y-4">
          <div>
            <label className="block  text-sm font-medium">Full Name</label>
            <input value={name} className='text-black' onChange={(e)=>setName(e.target.value)} type="text" className="w-full border p-2 rounded" />
          </div>
          <div>
            <label className="block  text-sm font-medium">Title</label>
            <input value={title} className='text-black'  onChange={(e)=>setTitle(e.target.value)} type="text" className="w-full border p-2 rounded" />
          </div>
          <div>
            <label className="block  text-sm font-medium">Bio</label>
            <textarea value={bio} className='text-black'  onChange={(e)=>setBio(e.target.value)} className="w-full border p-2 rounded"></textarea>
          </div>
          <div>
            <label className="block  text-sm font-medium">Website</label>
            <input value={website} className='text-black'  onChange={(e)=>setWebsite(e.target.value)} type="url" className="w-full border p-2 rounded" />
          </div>
