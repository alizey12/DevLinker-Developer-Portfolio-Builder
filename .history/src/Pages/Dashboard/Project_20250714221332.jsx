...
const Project = () => {
  ...
  // Enhanced theme classes with better color contrast
  const themeClasses = {
    bg: theme === 'dark' 
    ...
    card: theme === 'dark' 
      ...
      : 'bg-white border-gray-200 hover:border-emerald-300 shadow-lg',
    sectionBorder: theme === 'dark' ? 'border-emerald-800' : 'border-emerald-200',
    tag: {
      react: theme === 'dark' 
        ? 'bg-emerald-800/40 text-emerald-300 border-emerald-700' 
        : 'bg-emerald-100 text-emerald-800 border-emerald-200',
      js: theme === 'dark' 
        ? 'bg-amber-800/30 text-amber-300 border-amber-700' 
  ...
  const reactProjects = [
    ...
    },
    {
      ...
      description: "Scientific calculator with history tracking and theme switching",
      tags: ["React", "Context API", "Math.js"],
    ...
    },
    {
      ...
      description: "Timed quizzes with multiple categories and score tracking",
      tags: ["React", "Open Trivia API", "Framer Motion"],
    ...
    {
      title: "Todo App",
      description: "Feature-rich todo application with drag-and-drop functionality",
  ...
  return (
    <div className={`min-h-screen py-12 px-4 sm:px-6 lg:px-8 ${themeClasses.bg} transition-colors duration-300`}>
      <div className="max-w-7xl mx-auto">
        ...
        <section className="mb-20">
          <h2 className={`text-2xl font-bold mb-8 pb-2 border-b ${themeClasses.sectionBorder} flex items-center`}>
            <span className={`px-3 py-1 rounded-lg mr-3 ${theme === 'dark' ? 'bg-emerald-700/40' : 'bg-emerald-100'} transition-colors duration-300`}>
              React
            </span>
            Projects
          </h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {reactProjects.map((project, index) => (
              <div 
                key={index}
                className={`rounded-xl overflow-hidden transition-all duration-300 hover:-translate-y-1 ${themeClasses.card} hover:shadow-xl`}
              >
                ...
                <div className="p-6">
                  ...
                  <div className="flex flex-wrap gap-2 mb-6">
                    {project.tags.map((tag, i) => (
                      <span 
                        key={i}
                        className={`px-3 py-1 text-sm rounded-full border transition-colors duration-300 ${themeClasses.tag.react}`}
                      >
                        {tag}
                      </span>
                    ))}
        ...
        <section>
          <h2 className={`text-2xl font-bold mb-8 pb-2 border-b ${themeClasses.sectionBorder} flex items-center`}>
            <span className={`px-3 py-1 rounded-lg mr-3 ${theme === 'dark' ? 'bg-amber-700/30' : 'bg-amber-100'} transition-colors duration-300`}>
              JavaScript
            </span>
            Projects
          </h2>
                ...
                <div className="p-6">
                  ...
                  <div className="flex flex-wrap gap-2 mb-6">
                    {project.tags.map((tag, i) => (
                      <span 
                        key={i}
                        className={`px-3 py-1 text-sm rounded-full border transition-colors duration-300 ${themeClasses.tag.js}`}
                      >
                        {tag}
                      </span>
                    ))}
                  ...
                  <div className="flex space-x-4">
                    ...
                    <a 
                      ...
                      target="_blank" 
                      rel="noopener noreferrer"
                      className={`flex items-center px-4 py-2 rounded-lg transition-colors ${theme === 'dark' ? 'border border-amber-600 text-amber-300 hover:bg-amber-800/30' : 'border border-amber-500 text-amber-600 hover:bg-amber-50'}`}
                    >
                      <FiGithub className="mr-2" /> View Code
                    </a>
                  </div>
