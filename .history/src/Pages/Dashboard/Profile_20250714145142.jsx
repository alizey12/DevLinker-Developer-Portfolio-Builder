    ...
    >
      ...
      >
        ...
        <motion.div className="lg:col-span-1 space-y-6" variants={cardVariants}>
          ...
          >
            <div className="flex flex-col items-center">
              ...
              <a href={savedProfile.website} target="_blank" rel="noopener noreferrer" className={`mt-4 ${themeClasses.text.accent}`}>
            ...
            <div className={`mt-8 pt-6 border-t ${themeClasses.border}`}>
              <div className="grid grid-cols-2 gap-4">
                {[{ label: "Skills", value: "12+" }, { label: "Projects", value: "5+" }].map((stat, i) => (
                  <motion.div
                    key={i}
            ...
            <div className={`mt-8 pt-6 border-t ${themeClasses.border}`}>
              ...
              <div className="flex justify-center space-x-6">
                {[{ icon: <FaLinkedin size={24} />, url: "https://www.linkedin.com/in/kinza-jamshaid-70212a215" }, { icon: <SiGmail size={24} />, url: "mailto:alizey121@gmail.com" }].map((social, i) => (
                  <motion.a
                    key={i}
                    href={social.url}
                    target="_blank"
                    rel="noopener noreferrer"
                    className={`${themeClasses.text.secondary} hover:${themeClasses.text.accent}`}
                    variants={itemVariants}
        ...
        <motion.div className="lg:col-span-2 space-y-6" variants={containerVariants}>
          {/* Professional Info */}
          <motion.div className={`rounded-2xl p-6 border shadow-lg ${themeClasses.card}`} variants={cardVariants} whileHover={{ y: -5 }}>
            ...
            {!isEditing ? (
              <motion.div className="grid grid-cols-1 md:grid-cols-2 gap-6" variants={containerVariants}>
                {[
                  ...
                  <motion.div
                    key={i}
                    className={`p-4 rounded-xl ${theme === 'dark' ? 'bg-emerald-700/20 hover:bg-emerald-600/30' : 'bg-emerald-100 hover:bg-emerald-200'} ${item.span === 2 ? 'md:col-span-2' : ''}`}
                    variants={itemVariants}
              ...
              <motion.form onSubmit={handleSubmit} className="space-y-6" variants={containerVariants}>
                  ...
                  {[
                    ...
                    <motion.div key={i} variants={itemVariants}>
                      ...
                      <input
                        ...
                        type="text"
                        className={`w-full rounded-lg px-4 py-3 focus:ring-2 focus:${accentColor}-500 ${theme === 'dark' ? 'bg-emerald-700/50 border-emerald-600/50 text-emerald-100' : 'bg-white border-emerald-300 text-emerald-800'}`}
                      />
                    </motion.div>
                  ))}
                ...
                <motion.div className="flex justify-end space-x-3" variants={itemVariants}>
                  ...
                  <motion.button
                    type="submit"
                    className={`px-6 py-2 rounded-lg shadow-lg ${themeClasses.button.primary}`}
                    whileHover={{ scale: 1.05 }}
                    whileTap={{ scale: 0.95 }}
                  >
                    Save Changes
              ...
              <motion.div variants={itemVariants}>
                ...
                <div className="flex flex-wrap gap-3">
                  {['JavaScript Enthusiast', 'React Specialist'].map((badge, i) => (
                    <motion.span
                      key={i}
                      className={`inline-flex items-center gap-2 px-4 py-2 rounded-full text-sm font-medium bg-gradient-to-r from-${accentColor}-600 to-${accentColor}-500 text-white`}
                      whileHover={{ scale: 1.05 }}
                    >
              ...
              <motion.div variants={itemVariants}>
                ...
                <div className="flex flex-wrap gap-2">
                  {['React', 'TailwindCSS', 'JavaScript', 'Node.js', 'Express', 'MongoDB', 'Git', 'REST APIs'].map((tag, i) => (
                    <motion.span
                      key={tag}
                      className={`px-3 py-1 rounded-full text-sm ${theme === 'dark' ? 'bg-emerald-700/50 hover:bg-emerald-600/50' : 'bg-emerald-200 hover:bg-emerald-300'} ${themeClasses.text.secondary}`}
                      variants={itemVariants}
                      custom={i}
                      initial="hidden"
                      animate="visible"
