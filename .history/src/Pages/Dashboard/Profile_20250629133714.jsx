...
const Profile = () => {
  ...
  return (
    ...
    <div className="flex flex-col md:flex-row gap-6">
      {/* Left: Profile Card */}
      <div className="w-full md:w-1/3 bg-gray-900 text-white  p-6 rounded-lg shadow">
        <img
      ...
      {/* social media  */}
      <div className='w-full my-6 rounded-md slashed-zero bg-gray-900 text-white p-6'>
      <h2 className="text-xl font-semibold mb-4">Social media</h2>
<ul className='flex'>
  <li className='p-2'><a href="https://www.linkedin.com/in/kinza-jamshaid" target="_blank" rel="noopener noreferrer"><FaLinkedin size={25} /></a> </li>
  <li className='p-2'>  <a href="mailto:alizey121@gmail.com" ><SiGmail size={25} /> </a></li>
</ul>
</div>

      <div className='w-full my-9 rounded-md slashed-zero bg-gray-900 text-white p-6'>
      <h2 className="text-xl font-semibold mb-4">Edit Profile</h2>
        <form className="space-y-4">
          <div>
            <label className="block  text-sm font-medium">Full Name</label>
            <input value={name} onChange={(e)=>setName(e.target.value)} type="text" className="w-full border text-black p-2 rounded" />
          </div>
          <div>
            <label className="block  text-sm font-medium">Title</label>
            <input value={title} onChange={(e)=>setTitle(e.target.value)} type="text" className="w-full border text-black p-2 rounded" />
          </div>
          <div>
            <label className="block  text-sm font-medium">Bio</label>
            <textarea value={bio} onChange={(e)=>setBio(e.target.value)} className="w-full border p-2 rounded text-black"></textarea>
          </div>
          <div>
            <label className="block  text-sm font-medium">Website</label>
            <input value={website} onChange={(e)=>setWebsite(e.target.value)} type="url" className="w-full border p-2 text-black rounded" />
          </div>
