...
const Skills = () => {
  ...
  // Enhanced theme classes with deep emerald gradient
  const themeClasses = {
    bg: theme === 'dark' 
    ...
    hover: theme === 'dark' 
      ? 'hover:bg-emerald-800/70 hover:shadow-emerald-900/30' 
  ...
  const cardVariants = {
    ...
    hover: {
      ...
      boxShadow: theme === 'dark' 
        ? '0 10px 25px -5px rgba(5, 150, 105, 0.2)' 
  ...
  return (
    <div className={`min-h-screen ${themeClasses.bg} py-12 px-4 sm:px-6 lg:px-8 transition-colors duration-300`} ref={ref}>
      <div className="max-w-7xl mx-auto">
        ...
        >
          <h1 className={`text-4xl md:text-5xl font-bold mb-4 ${themeClasses.text.primary}`}>My Skills</h1>
        ...
        >
          {/* Radial Chart */}
          <motion.div 
            className={`rounded-2xl p-6 border ${themeClasses.card} transition-all duration-300 ${themeClasses.hover}`}
            ...
            <div className="relative w-64 h-64 mx-auto">
              ...
              <div className="absolute inset-0 flex items-center justify-center">
                <div className="text-center">
                  ...
                  <p className={`${themeClasses.text.accent} font-medium`}>Languages</p>
                </div>
              </div>
            </div>
          ...
          >
            ...
            <div className="space-y-6">
              <motion.div 
                className={`p-4 rounded-lg ${theme === 'dark' ? 'bg-emerald-800/40' : 'bg-emerald-100'} transition-colors duration-300 hover:bg-opacity-80`}
                variants={itemVariants}
          ...
          >
            ...
            <div className="flex items-center justify-between mb-4">
              <motion.div 
                className={`p-3 rounded-lg ${theme === 'dark' ? 'bg-emerald-800/40' : 'bg-emerald-100'} transition-colors duration-300 hover:bg-opacity-80`}
                variants={itemVariants}
                whileHover={{ scale: 1.03 }}
              >
            ...
            <motion.div 
              className={`p-4 rounded-lg ${theme === 'dark' ? 'bg-emerald-800/40' : 'bg-emerald-100'}`}
              variants={itemVariants}
            >
              <div className="grid grid-cols-7 gap-1">
                {Array.from({ length: 35 }).map((_, i) => (
                  <motion.div 
                    key={i}
                    className={`h-3 rounded-sm ${i % 5 === 0 ? `bg-${accentColor}-500` : theme === 'dark' ? 'bg-emerald-700/50' : 'bg-emerald-300/50'}`}
            ...
            <motion.a 
              ...
              className={`inline-flex items-center mt-4 ${themeClasses.text.accent} hover:opacity-80 transition-opacity duration-300`}
              variants={itemVariants}
              whileHover={{ x: 5 }}
            >
        ...
        >
          {skillCategories.map((category, index) => (
            <motion.div 
              key={index} 
              className={`rounded-2xl overflow-hidden border transition-all duration-300 ${themeClasses.card} ${themeClasses.hover}`}
              variants={cardVariants}
              whileHover="hover"
            >
              <div className={`p-6 border-b flex items-center ${themeClasses.border}`}>
                <div className={`mr-3 p-2 rounded-lg ${theme === 'dark' ? 'bg-emerald-800/40' : 'bg-emerald-100'}`}>
                  {category.icon}
                </div>
              ...
              <div className="p-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                {category.skills.map((skill, i) => (
                  ...
                  >
                    ...
                    <div className={`w-full h-3 rounded-full ${theme === 'dark' ? 'bg-emerald-800/40' : 'bg-emerald-100'}`}>
                      <motion.div 
        ...
        {/* Additional Skills */}
        <motion.div 
          className={`mt-12 rounded-2xl p-6 border ${themeClasses.card} transition-all duration-300 ${themeClasses.hover}`}
          variants={cardVariants}
          whileHover="hover"
        >
          ...
          <div className="flex flex-wrap gap-3">
            {['Redux', 'Context API', 'Jest', 'React Testing Library', 'Webpack', 'Vite', 'Figma', 'Agile Methodologies', 'JIRA', 'Postman', 'ES6+', 'Responsive Design', 'CSS Grid', 'Flexbox', 'RESTful APIs'].map((skill, i) => (
              <motion.span 
                key={skill}
                className={`px-4 py-2 rounded-full border transition-colors duration-300 ${theme === 'dark' ? 'bg-emerald-800/40 border-emerald-700 hover:bg-emerald-700/60' : 'bg-emerald-100 border-emerald-200 hover:bg-emerald-200'}`}
                variants={itemVariants}
                whileHover={{ scale: 1.05 }}
                transition={{ delay: i * 0.03 }}
              >
                <span className={themeClasses.text.secondary}>{skill}</span>
              </motion.span>
            ))}
          </div>
        </motion.div>
